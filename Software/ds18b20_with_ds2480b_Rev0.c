...
//power up omits the command mode command
int ds2480b_startup(fd)
{
	...
	n = write(fd, tbuf, 1);
	if (n != 1) {
		fprintf(stderr, "ds2480b_startup: failed 1-Wire reset command\n");
		return(-1);
	}
...
/*------------------------------------------------------------------------------
	...
	serialBreak(int fd) - Send a BREAK to the DS2480B line driver
